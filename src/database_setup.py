...
class DatabaseSetup:
    ...
    def display_schema(self):
        ...
        for row in self.cursor.fetchall():
            print(
                f"  {row[1]:<15} {row[2]:<10} {'NOT NULL' if row[3] else ''} {f'DEFAULT {row[4]}' if row[4] else ''}"
            )
    ...
    def run_sample_queries(self):
        ...
        for row in self.cursor.fetchall():
            print(
                f"  Ticket #{row[0]} | {row[1]:<20} | {row[3]:<11} | {row[4]:<6} | {row[2][:40]}"
            )
...
def main():
    ...
    db_file = project_root / "support.db"

    db = DatabaseSetup(str(db_file))
    ...
    try:
        ...
        if response == "y":
            ...
            query_response = input(
                "\nWould you like to run sample queries? (y/n): "
            ).lower()
            if query_response == "y":
                ...
                for row in db.cursor.fetchall():
                    print(f"  {row}")
                print(
                    f"  ... ({db.cursor.execute('SELECT COUNT(*) FROM customers').fetchone()[0]} total)"
                )
                ...
                for row in db.cursor.fetchall():
                    print(f"  {row}")
                print(
                    f"  ... ({db.cursor.execute('SELECT COUNT(*) FROM tickets').fetchone()[0]} total)"
                )
